...
export const CartDrawer = ({ buttonClassName, iconClassName }: Props) => {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <button
          className={buttonClassName ?? "relative inline-flex items-center justify-center h-10 w-10 rounded-full hover:bg-foreground/5 transition"}
          aria-label="Abrir carrinho"
        >
          <ShoppingBag className={iconClassName ?? "h-5 w-5"} />
          {totalItems > 0 && (
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader>
          ...
          <SheetDescription>
            {totalItems === 0 ? "Sua sacola está vazia" : `${totalItems} item${totalItems > 1 ? "s" : ""}`}
          </SheetDescription>
        </SheetHeader>
            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-medium mt-1">{formatBRL(item.price.amount, item.price.currencyCode)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-display font-medium">{formatBRL(totalPrice, currency)}</span>
                </div>
