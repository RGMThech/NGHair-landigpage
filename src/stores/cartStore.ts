...
function isCartNotFound(errors: Array<{ message: string }>) {
  return errors.some((e) =>
    /(cart not found|does not exist)/i.test(e.message),
  );
}
...
async function createShopifyCart(item: CartItem) {
  ...
  if (errors.length) { console.error("cart create", errors); return null; }
...
// helper to silence unused import for STOREFRONT_QUERY in some builds
void STOREFRONT_QUERY;